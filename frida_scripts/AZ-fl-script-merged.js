...
(function() {
  ...
  Java.perform(function() {
    try {
      // Native bridge class
      var RBn = Java.use("com.scottyab.rootbeer.RootBeerNative");
      // Report "no root" from native check
      if (RBn.checkForRoot) {
        // Signature: checkForRoot(Object[])
        try { RBn.checkForRoot.overload('[Ljava.lang.Object;').implementation = function(arr) { return 0; }; } catch (e) { 
          // fallback if overload signature differs
          RBn.checkForRoot.implementation = function() { return 0; };
        }
      }
      // Pretend native lib is present/okay
      if (RBn.a) { RBn.a.implementation = function() { return true; }; }
      if (RBn.setLogDebugMessages) { RBn.setLogDebugMessages.implementation = function(z) { return 0; }; }
      console.log("[*] RootBeerNative hooks installed");
    } catch (e) {
    ...
    try {
      ...
      var patched = 0;
      function safeFalseReplace(klass, method) {
        if (klass[method]) { klass[method].implementation = function() { return false; }; patched++; }
      }
      safeFalseReplace(RB, "isRooted");
      safeFalseReplace(RB, "isRootedWithBusyBoxCheck");
      safeFalseReplace(RB, "isRootedWithoutBusyBoxCheck");
      safeFalseReplace(RB, "detectRootManagementApps");
      safeFalseReplace(RB, "detectPotentiallyDangerousApps");
      safeFalseReplace(RB, "detectTestKeys");
      safeFalseReplace(RB, "checkForBinary");
      safeFalseReplace(RB, "checkForSuBinary");
      safeFalseReplace(RB, "checkForBusyBoxBinary");
      safeFalseReplace(RB, "checkSuExists");
      if (patched > 0) {
        console.log("[*] RootBeer Java hooks installed (" + patched + " methods patched)");
      }
    } catch (e) {
    ...
    try {
      ...
      if (Build.TAGS) {
        ...
        console.log("[*] Build.TAGS patched to release-keys");
      }
    } catch (e) {}
  });
})();
